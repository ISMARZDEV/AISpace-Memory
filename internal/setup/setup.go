...
//     (patching AISPACE_MEN_BIN to bake in the absolute binary path as a final
...
//     then writes a durable MCP config to ~/.claude/mcp/aispace-men.json using the
...
const claudeCodeMarketplace = "aispace/men"

// claudeCodeMCPTools are the MCP tool names registered by the aispace-men plugin
...
var claudeCodeMCPTools = []string{
	"mcp__plugin_aispace-men_aispace-men__mem_capture_passive",
	"mcp__plugin_aispace-men_aispace-men__mem_context",
	"mcp__plugin_aispace-men_aispace-men__mem_get_observation",
	"mcp__plugin_aispace-men_aispace-men__mem_save",
	"mcp__plugin_aispace-men_aispace-men__mem_save_prompt",
	"mcp__plugin_aispace-men_aispace-men__mem_search",
	"mcp__plugin_aispace-men_aispace-men__mem_session_end",
	"mcp__plugin_aispace-men_aispace-men__mem_session_start",
	"mcp__plugin_aispace-men_aispace-men__mem_session_summary",
	"mcp__plugin_aispace-men_aispace-men__mem_suggest_topic_key",
	"mcp__plugin_aispace-men_aispace-men__mem_update",
}

const memoryProtocolMarkdown = `## Aispace-Men Persistent Memory — Protocol
...
with numbered items. Aispace-Men will automatically extract and save these as observations.
...
const compactPromptMarkdown = `You are compacting a coding session that uses Aispace-Men persistent memory.
...
// patchAispaceMenBINLine rewrites the ENGRAM_BIN constant declaration in the
// plugin source so the installed copy contains an absolute fallback path.
//
// Original line in source:
//
//	const ENGRAM_BIN = process.env.ENGRAM_BIN ?? "aispace-men"
//
// Patched line in installed copy:
//
//	const ENGRAM_BIN = process.env.ENGRAM_BIN ?? Bun.which("aispace-men") ?? "/abs/path/aispace-men"
//
// Priority (left to right, first truthy wins):
//  1. ENGRAM_BIN env var — explicit user override, always respected.
//  2. Bun.which("aispace-men") — runtime PATH lookup; works in interactive shells.
//  3. Absolute baked-in path — works in headless/systemd where PATH is stripped.
//
// If absBin is already bare "aispace-men" (os.Executable fallback) we don't add it
// as the third fallback because it would be redundant with Bun.which("aispace-men").
func patchAispaceMenBINLine(src []byte, absBin string) []byte {
	const marker = `const ENGRAM_BIN = process.env.ENGRAM_BIN ?? "aispace-men"`

	var replacement string
	if absBin == "aispace-men" {
		// os.Executable failed — add Bun.which but no baked-in absolute path
		replacement = `const ENGRAM_BIN = process.env.ENGRAM_BIN ?? Bun.which("aispace-men") ?? "aispace-men"`
	} else {
		// Normal case: bake in the absolute path as final fallback
		replacement = fmt.Sprintf(
			`const ENGRAM_BIN = process.env.ENGRAM_BIN ?? Bun.which("aispace-men") ?? %q`,
			absBin,
		)
	}
...
func installOpenCode() (*Result, error) {
	...
	data, err := openCodeReadFile("plugins/opencode/aispace-men.ts")
	if err != nil {
		return nil, fmt.Errorf("read embedded aispace-men.ts: %w", err)
	}
	...
	// process.env.ENGRAM_BIN (explicit user override) and Bun.which("aispace-men")
	...
	data = patchAispaceMenBINLine(data, resolveAispaceMenCommand())

	dest := filepath.Join(dir, "aispace-men.ts")
	if err := openCodeWriteFileFn(dest, data, 0644); err != nil {
		return nil, fmt.Errorf("write %s: %w", dest, err)
	}

	// Register aispace-men MCP server in opencode.json
	files := 1
	if err := injectOpenCodeMCPFn(); err != nil {
		// Non-fatal: plugin works, MCP just needs manual config
		cmd := resolveAispaceMenCommand()
		...
		fmt.Fprintf(os.Stderr, "  \"aispace-men\": { \"type\": \"local\", \"command\": [%q, \"mcp\", \"--tools=agent\"], \"enabled\": true }\n", cmd)
	} else {
		files = 2
	}
...
// injectOpenCodeMCP adds the aispace-men MCP server entry to opencode.json.
// It reads the existing config, adds/updates the aispace-men entry under "mcp",
...
func injectOpenCodeMCP() error {
	...
	// Check if aispace-men is already registered
	if _, exists := mcpBlock["aispace-men"]; exists {
		return nil // already registered, nothing to do
	}

	// Add aispace-men MCP entry (agent profile — only tools agents need).
	// Use resolveAispaceMenCommand() so Windows users (and headless Linux setups
	// where PATH is not inherited) get the absolute binary path.
	aispaceMenEntry := map[string]interface{}{
		"type":    "local",
		"command": []string{resolveAispaceMenCommand(), "mcp", "--tools=agent"},
		"enabled": true,
	}
	entryJSON, err := jsonMarshalFn(aispaceMenEntry)
	if err != nil {
		return fmt.Errorf("marshal aispace-men entry: %w", err)
	}
	mcpBlock["aispace-men"] = json.RawMessage(entryJSON)
...
func installClaudeCode() (*Result, error) {
	...
	// Step 1: Try to add marketplace (optional - may fail if repo doesn't exist)
	// For aispace-men, we skip marketplace and just write MCP config directly
	_ = claudeBin // silence unused variable warning

	// Step 2: Write a durable user-level MCP config at ~/.claude/mcp/aispace-men.json
	// with the absolute binary path. This is the primary installation method.
	files := 0
	if err := writeClaudeCodeUserMCPFn(); err != nil {
		...
		fmt.Fprintf(os.Stderr, "warning: could not write user MCP config (~/.claude/mcp/aispace-men.json): %v\n", err)
		fmt.Fprintf(os.Stderr, "  The plugin is installed but MCP may not start on Windows if aispace-men is not in PATH.\n")
	} else {
		files = 1
	}
...
// claudeCodeUserMCPPath returns the path for the aispace-men MCP config in the
// user-level MCP directory.
func claudeCodeUserMCPPath() string {
	return filepath.Join(claudeCodeMCPDir(), "aispace-men.json")
}

// writeClaudeCodeUserMCP writes ~/.claude/mcp/aispace-men.json with the absolute
// path to the aispace-men binary. This is idempotent — it always writes (overwrites)
...
// AddClaudeCodeAllowlist adds aispace-men MCP tool names to ~/.claude/settings.json
...
// 	}
//
// 	aispaceMenEntry := map[string]any{
// 		"command": resolveAispaceMenCommand(),
// 		"args":    []string{"mcp", "--tools=agent"},
// 	}
// 	entryJSON, err := jsonMarshalFn(aispaceMenEntry)
// 	if err != nil {
// 		return fmt.Errorf("marshal aispace-men entry: %w", err)
// 	}
// 	mcpServers["aispace-men"] = json.RawMessage(entryJSON)
//
...
// resolveAispaceMenCommand returns the absolute path to the aispace-men binary.
...
// Falls back to bare "aispace-men" only if os.Executable() itself fails.
func resolveAispaceMenCommand() string {
	exe, err := osExecutable()
	if err != nil {
		return "aispace-men" // fallback to PATH-based name
	}
	if resolved, err := filepath.EvalSymlinks(exe); err == nil {
		exe = resolved
	}
...
// Previous versions of aispace-men added this line, but it causes Gemini CLI to look
// for system.md relative to CWD instead of ~/.gemini/. Best-effort cleanup.
// NOTE: Gemini CLI is not supported in v1.0, so this function is commented out.
// func removeGeminiEnvOverride() {
// 	envPath := filepath.Join(filepath.Dir(filepath.Join(homeDir(), ".gemini", "settings.json")), ".env")
...
// 	}
//
// 	updated := upsertCodexAispaceMenBlock(string(data))
...
// upsertCodexBlock is kept for compatibility but returns empty string since Codex is not supported in v1.0.
func upsertCodexBlock(content string) string {
	return content
}
