// Package mcp implements the Model Context Protocol server for aispace-men.
//
// This exposes memory tools via MCP stdio transport so ANY agent
// (OpenCode, Claude Code, Cursor, Windsurf, etc.) can use aispace-men's
// persistent memory just by adding it as an MCP server.
//
// Tool profiles allow agents to load only the tools they need:
//
//	aispace-men mcp                    → all 15 tools (default)
//	aispace-men mcp --tools=agent      → 11 tools agents actually use (per skill files)
//	aispace-men mcp --tools=admin      → 4 tools for TUI/CLI (delete, stats, timeline, merge)
//	aispace-men mcp --tools=agent,admin → combine profiles
//	aispace-men mcp --tools=mem_save,mem_search → individual tool names
package mcp
...
import (
	...
	projectpkg "github.com/aispace/men/internal/project"
	"github.com/aispace/men/internal/store"
...
// serverInstructions tells MCP clients when to use aispace-men's tools.
// 6 core tools are eager (always in context). The rest are deferred
// and require ToolSearch to load.
const serverInstructions = `aispace-men provides persistent memory that survives across sessions and compactions.
	...
	if shouldRegister("mem_merge_projects", allowlist) {
		srv.AddTool(
			mcp.NewTool("mem_merge_projects",
				mcp.WithDescription("Merge memories from multiple project name variants into one canonical name. Use when you discover project name drift (e.g. 'aispace-men' and 'aispace-men' should be the same project). DESTRUCTIVE — moves all records from source names to the canonical name."),
				mcp.WithDeferLoading(true),
				...
				mcp.WithString("from",
					mcp.Required(),
					mcp.Description("Comma-separated list of project names to merge FROM (e.g. 'aispace-men,aispace-men-memory,ENGRAM')"),
				),
				mcp.WithString("to",
					mcp.Required(),
					mcp.Description("The canonical project name to merge INTO (e.g. 'aispace-men')"),
				),
			),
